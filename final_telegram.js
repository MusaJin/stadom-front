    ...
    if (response.data.ok) {
      ...
      const textsAndPhotos = await Promise.all(
        Object.values(groupedMessages).map(async (group) => {
          ...
          const photoUrls = validPhotos.map((fileId) => `/refs/${fileId}`).join(",");

          try {
            const apiUrl = `http://musaku0d.beget.tech/api/add_news.php?title=${encodeURIComponent(caption)}&text=${encodeURIComponent(caption)}&img=${validPhotos.join(",")}&date=${date}`;
